...
export default function App() {
  ...
  const enviarDadosParaAPI = async () => {
    dados.forEach(async data => {
      ...
      try {
        const resposta = await api.post('/cadastro/novo', data);
        console.log('Resposta da API:', resposta.data);
        Alert.alert('Dados enviados com sucesso!');
      } catch (erro) {
        console.error('Erro ao enviar dados para a API:', erro);
        Alert.alert('Erro ao enviar os dados. Por favor, tente novamente.');
      }
    });
  };
    ...
    <View style={styles.container}>
      ...
      {dados ? (
        <View style={styles.dadosContainer}>
          ...
          <Text>{JSON.stringify(dados)}</Text>
          <TouchableOpacity style={styles.buttonEnviar} onPress={enviarDadosParaAPI}>
            <Text style={styles.buttonText}>Enviar Dados</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <Text style={styles.infoText}>Nenhum arquivo selecionado</Text>
      )}
    </View>
      <StatusBar style="auto" />
    </View>
